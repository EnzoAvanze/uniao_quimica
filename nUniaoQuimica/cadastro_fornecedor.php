...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($CNPJ) {
        ...
        $sql = "UPDATE fornecedor SET Nome_Fornecedor='$Nome_Fornecedor', Email='$Email', telefone='$telefone' Observacoes='$Observacoes' Endereco='$Endereco'";
        if($imagem) {
            $sql .= ", imagem='$imagem'";
        }
        $sql .= " WHERE id='$id'";
        ...
        $sql = "INSERT INTO fornecedor (Nome_Fornecedor, Email, telefone, Observacoes, Endereco, imagem) VALUES ('$Nome_Fornecedor', '$email', '$telefone', '$Observacoes', 'Endereco', '$imagem')";
...
if (isset($_GET['delete_id'])) {
    ...
    $check_produtos = $conn->query("SELECT COUNT(*) as count FROM produtos WHERE CNPJ = '$delete_id'")->fetch_assoc();
...
if (isset($_GET['edit_id'])) {
    ...
    $fornecedor = $conn->query("SELECT * FROM fornecedor WHERE CNPJ='$edit_id'")->fetch_assoc();
}
?>
...
<body>
    <div class="container" style="width: 900px;">
        ...
        <form method="post" action="" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?php echo $fornecedor['CNPJ'] ?? ''; ?>">
            ...
            <input type="text" name="Observacoes" value="<?php echo $fornecedor['Observacoes'] ?? ''; ?>">
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <a href="?edit_id=<?php echo $row['CNPJ']; ?>">Editar</a>
                    <a href="?delete_id=<?php echo $row['CNPJ']; ?>" onclick="return confirm('Tem certeza que deseja excluir?')">Excluir</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </table>
